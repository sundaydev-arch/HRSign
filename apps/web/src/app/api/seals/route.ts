...
/** Reject stub/test 1×1 PNGs — a usable company seal needs real pixels. */
const MIN_SEAL_EDGE = 64;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!size || size.width < MIN_SEAL_EDGE || size.height < MIN_SEAL_EDGE) {
      throw new ApiError(400, "SEAL_IMAGE_TOO_SMALL", { minPx: MIN_SEAL_EDGE });
    }
