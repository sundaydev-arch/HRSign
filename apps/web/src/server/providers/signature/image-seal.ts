...
export class ImageSealProvider implements SignatureProvider {
  ...
  async sign(input: {
    ...
    const { source, signer, watermarkText, claimedVersion } = input;
    ...
    if (!Number.isInteger(claimedVersion) || claimedVersion <= source.version) {
      throw new Error("ImageSealProvider.sign requires claimedVersion > source.version");
    }
    ...
    // 3. Use the pre-claimed version number (spec 3.2: key carries version).
    const newVersionNumber = claimedVersion;
